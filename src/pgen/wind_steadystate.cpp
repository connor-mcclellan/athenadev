...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  Real dens_base = mesa_in(RHO, 2);
  Real vinflow = mesa_in(VELOCITY, 2);
  //Real Fr_com_target = mesa_in(LUMINOSITY, 2) / (SQR(pcoord->x1v(2)));
  ...
  for (int k = ks; k <= ke; k++) {
    for (int j = js; j <= je; j++) {
      for (int i = is - NGHOST; i <= ie + NGHOST; i++) {
        ...
        Real rho = dens_base * std::pow(x1v / x1min, -2);
        phydro->u(IDN,k,j,i) = rho;
        phydro->u(IM1,k,j,i) = vinflow * rho;
        phydro->u(IM2,k,j,i) = 0.0;
        phydro->u(IM3,k,j,i) = 0.0;

        // Get gas temperature that will put the gas and radiation
        // roughly in equilibrium
        Real tau = SQR(x1min) * dens_base * (1./x1v - 1./x1max);
        ...
        if (IM_RADIATION_ENABLED || NR_RADIATION_ENABLED) {
          ...
          for (int ifr = 0; ifr < pnrrad->nfreq; ++ifr) {
            ...
            SetLabIr(pnrrad->nang, 0., Er_com_target, Fr_com_target, k, j, i,
                lab_ir, lab_ir1, mu, wmu);
          }
        }
      }
    }
    ...
    for (int j=jl; j<=ju; ++j) {
      ...
      for (int i=il-1; i>=il-ngh; --i) {
        ...
        for (int n=0; n<=NHYDRO; n++) {
          if (n == IDN) {
            prim(n, k, j, i) = mesa_in(RHO, 2);
          } else if (n == IPR) {
            Real x = pmb->pcoord->x1v(i);
            Real tgas = pow(Er_above,0.25);
            prim(n, k, j, i) = prim(IDN,k,j,il) * tgas;
          } else if (n == IVX) {
            prim(n, k, j, i) = mesa_in(VELOCITY, 2);
          } else {
  ...
  for (int k = ks; k <= ke; ++k) {
    for (int j = js; j <= je; ++j) {
      ...
      for (int i=is-1; i>=is-ngh; --i) {
        ...
        Real cs2 = std::pow(Er_above,0.25)/w(IDN,k,j,i);
        ...
        SetLabIr(pnrrad->nang, beta, Er, Fr_target, k, j, i, lab_ir, lab_ir1, mu, wmu);
      }
    }
  }
}
